...
class Doc2Vec(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default(), tf.device('/cpu:0'):
            ...
            norm_w = tf.sqrt(tf.reduce_sum(tf.square(self.word_embeddings), axis=1, keep_dims=True))
            self.normalized_word_embeddings = self.word_embeddings / norm_w

            norm_d = tf.sqrt(tf.reduce_sum(tf.square(self.doc_embeddings), axis=1, keep_dims=True))
